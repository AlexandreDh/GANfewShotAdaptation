...
def setup_training_loop_kwargs(
    ...
    if metricdata is not None:
        ...
        try:
            metric_dataset = dnnlib.util.construct_class_by_name(
                **args.metric_dataset_kwargs)  # subclass of training.dataset.Dataset
    ...
    cfg_specs = {
        ...
        'paper512': dict(ref_gpus=8, kimg=25000, mb=64, mbstd=8, fmaps=1, lrate=0.0025, gamma=0.5, ema=20, ramp=None,
                         map=8),
        'paper1024': dict(ref_gpus=8, kimg=25000, mb=32, mbstd=4, fmaps=1, lrate=0.002, gamma=2, ema=10, ramp=None,
                          map=8),
...
def subprocess_fn(rank, args, temp_dir):
    ...
    args.augment_pipe = None

    if (args.augment_kwargs is not None) and (args.augment_p > 0 or args.apa_target is not None):
        args.augment_pipe = dnnlib.util.construct_class_by_name(**args.augment_kwargs).train().requires_grad_(False).to(
            device)  # subclass of torch.nn.Module
...
@click.option('--use_tpu', help='Use TPU for training [default: false]', type=bool, metavar='BOOL')
...
@click.option('--with-dataaug', help='Enable standard data augmentations for the discriminator inputs [default: false]',
...
def main(ctx, outdir, dry_run, **config_kwargs):
    ...
    print(f'Using TPU:          {args.use_tpu}')
