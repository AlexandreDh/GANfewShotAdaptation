...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    gw = np.clip(7680 // training_set.image_shape[2], 7, 8)  # original clip max=32
    gh = np.clip(4320 // training_set.image_shape[1], 4, 4)  # original clip max=32
...
def training_loop(
        ...
        num_gpus=1,  # Number of GPUs/XLA Device participating in the training.
        ...
        with_dataaug=False,
        # Enable standard data augmentations for the discriminator inputs? Usually improve further thanks to the compatibility.
    ...
    if running_xla:
        try:
            ...
            print("Could not import torch_xla")
            raise

    print_fun = print if not running_xla else xm.master_print

    # Initialize.
    start_time = time.time()
    device = torch.device('cuda', rank) if not running_xla else xm.xla_device()
    ...
    if not running_xla:
        ...
        if not augment_pipe is None:
            augment_pipe = augment_pipe.to(device)
    ...
    # Load training set.
    if rank == 0:
        print_fun('Loading training set...')
    training_set = dnnlib.util.construct_class_by_name(**training_set_kwargs)  # subclass of training.dataset.Dataset
    training_set_sampler = misc.InfiniteSampler(dataset=training_set, rank=rank, num_replicas=num_gpus,
                                                seed=random_seed)

    train_loader = torch.utils.data.DataLoader(dataset=training_set, sampler=training_set_sampler,
    ...
    if rank == 0:
        ...
        print_fun('Label shape:', training_set.label_shape)
        print_fun()
    ...
    if rank == 0:
        ...
        img = misc.print_fun_module_summary(G, [z, c], print=print_fun)
        misc.print_fun_module_summary(D, [img, c], print=print_fun)
    ...
    for name, module in [('G_mapping', G.mapping), ('G_synthesis', G.synthesis), ('D', D), (None, G_ema),
        ...
        if not running_xla and (num_gpus > 1) and (module is not None) and len(list(module.parameters())) != 0:
    ...
    while True:
        ...
        # Fetch training data.
        if running_xla:
            phase_real_img, phase_real_c, all_gen_z, all_gen_c = data_fetch()
        else:
            with torch.autograd.profiler.record_function('data_fetch'):
                phase_real_img, phase_real_c, all_gen_z, all_gen_c = data_fetch()

        def remove_nan_weight(phase):
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            if phase.start_event is not None and not running_xla:
        ...
        # print_fun status line, accumulating the same information in stats_collector.
        ...
        if not running_xla:
            ...
            fields += [
                f"time {dnnlib.util.format_time(tick_end_time - start_time):<12s}"]
            fields += [
                f"sec/tick {tick_end_time - tick_start_time:<7.1f}"]
            fields += [
                f"sec/kimg {(tick_end_time - tick_start_time) / (cur_nimg - tick_start_nimg) * 1e3:<7.2f}"]
        ...
        if not running_xla and (snapshot_data is not None) and (len(metrics) > 0):
            ...
            for metric in metrics:
                result_dict = metric_main.calc_metric(metric=metric, G=snapshot_data['G_ema'],
                                                      dataset_kwargs=metric_dataset_kwargs, num_gpus=num_gpus,
