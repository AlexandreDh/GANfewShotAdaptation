...
class FewShotAdaptationLoss(Loss):
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain, is_subspace):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws, _ = self.run_G(gen_z, gen_c, sync=(sync and not do_Gpl), is_subspace=is_subspace) # May get synced by Gpl.
                # Update pseudo data
                self.pseudo_data = gen_img.detach()
        ...
        if do_Gpl:
            with torch.autograd.profiler.record_function('Gpl_forward'):
                batch_size = gen_z.shape[0] // self.pl_batch_shrink
                gen_img, gen_ws, _ = self.run_G(gen_z[:batch_size], gen_c[:batch_size], sync=sync, is_subspace=is_subspace)
                pl_noise = torch.randn_like(gen_img) / np.sqrt(gen_img.shape[2] * gen_img.shape[3])
                with torch.autograd.profiler.record_function('pl_grads'), conv2d_gradfix.no_weight_gradients():
                    pl_grads = torch.autograd.grad(outputs=[(gen_img * pl_noise).sum()], inputs=[gen_ws], create_graph=True, only_inputs=True)[0]
                pl_lengths = pl_grads.square().sum(2).mean(1).sqrt()
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                ...
                gen_logits = self.run_D(gen_img, gen_c, sync=False, is_subspace=is_subspace) # Gets synced by loss_Dreal.
                training_stats.report('Loss/scores/fake', gen_logits)
                training_stats.report('Loss/signs/fake', gen_logits.sign())
                loss_Dgen = torch.nn.functional.softplus(gen_logits) # -log(1 - sigmoid(gen_logits))
            with torch.autograd.profiler.record_function('Dgen_backward'):
